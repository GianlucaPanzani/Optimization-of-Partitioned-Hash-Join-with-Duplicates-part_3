...
Strong and weak scalability are measured only with the selected workload-balanced loop implementation, using the best fixed parameters found in the reduced grid. These plots should therefore be read as scalability of the chosen OpenMP loop solution, not as another comparison among all implementations. The loop version is used because has the more balanced and expected behavior.
...
Weak scaling depends strongly on the dataset distribution. The uniform case scales best overall: it reaches about \(1.79\times\) scaled speedup at 16 threads and remains at \(1.33\times\) at 32 threads. The \texttt{skewed\_90\_5} case is competitive up to 16 threads, but drops to about \(0.97\times\) at 32 threads. The strongest skew, \texttt{skewed\_90\_1}, degrades much earlier and reaches only about \(0.36\times\) at 32 threads, because increasing \(N\) also enlarges the hot partitions and their duplicate-key work.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.74\linewidth]{../img/013_weak_scalability_scaled_speedup_dedicated.png}
...
Strong scaling also separates the datasets clearly. Uniform data benefits the most from extra threads and grows up to about \(6.67\times\) at 64 threads. The \texttt{skewed\_90\_5} dataset improves up to about \(5.34\times\) at 32 threads, then loses performance at 64 threads because the remaining hot-partition work does not scale enough to compensate for the extra overhead. The \texttt{skewed\_90\_1} dataset scales poorly across the whole range and reaches only about \(1.75\times\), showing that the strongest skew leaves too little independent work once a few hot partitions dominate the join.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.74\linewidth]{../img/011_strong_scalability_speedup_dedicated.png}
